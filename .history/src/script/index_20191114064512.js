...
const onSelectCard = e => {
    ...
    if (!card.classList.contains('active')) {
        ...
        console.dir(card.parentElement.children);
        card.parentElement.children.forEach(element => {
            element.classList.remove('active');
        })
        card.classList.add('active');
        transactionsLists.children.forEach(list => {
            list.classList.remove('active');
            document.querySelector(`.tranasctions-list[card-id=${cardId}]`).classList.add('active');
        })
    }
}
